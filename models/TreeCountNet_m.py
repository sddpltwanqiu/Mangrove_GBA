...
class TreeCountNet(nn.Module):
	def __init__(self, args):
		...
		self.pool3 = nn.MaxPool2d(kernel_size=16, stride=16)
		...
		self.sconv1 = ConvBNReLU(in_channels=32, out_channels=64, kernel_size=3)
		self.sup1 = nn.ConvTranspose2d(in_channels=256, out_channels=64, kernel_size=4, stride=4)
		self.sconv2 = ConvBNReLU(in_channels=128, out_channels=128, kernel_size=3)
		self.sconv3 = ConvBNReLU(in_channels=256, out_channels=256, kernel_size=3)
		self.sconv31 = ConvBNReLU(in_channels=64, out_channels=256, kernel_size=1)
		self.sconv4 = ConvBNReLU(in_channels=512, out_channels=256, kernel_size=3)
		self.sconv41 = ConvBNReLU(in_channels=32, out_channels=256, kernel_size=1)
		self.sconv5 = ConvBNReLU(in_channels=512, out_channels=128, kernel_size=1)
		self.sconv6 = ConvBNReLU(in_channels=128, out_channels=32, kernel_size=1)
		self.shadow = nn.Conv2d(32, 2, kernel_size=1)
	...
	def forward(self, x):
		...
		shadow1 = self.sconv1(out0_0)
		shadow1 = self.sconv2(torch.cat([self.sup1(out3_1), self.pool(shadow1)], 1))
		shadow1 = self.sconv3(torch.cat([out2_2, self.pool(shadow1)], 1))
		shadow1 = self.sconv4(torch.cat([self.sconv31(self.pool2(out1_3)), self.pool(shadow1)], 1))
		shadow1 = self.sconv5(torch.cat([self.sconv41(self.pool3(out0_4)), self.pool(shadow1)], 1))
		shadow1 = self.sconv6(shadow1)
		...
		if self.args.deepsupervision:
			...
			shadow = self.shadow(shadow1)
			return output1, output2, output3, output4, shadow
